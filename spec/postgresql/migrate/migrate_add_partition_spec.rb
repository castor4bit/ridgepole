...
describe 'Ridgepole::Client#diff -> migrate', condition: '>= 6.0' do
  ...
  context 'when add hash partition with multiple columns' do
    let(:actual_dsl) do
      erbh(<<-ERB)
        create_table "hash_partitions", id: false, force: :cascade, options: "PARTITION BY HASH(id,logdate)" do |t|
          t.integer "id", null: false
          t.date "logdate", null: false
        end
        add_partition "hash_partitions", :hash, [:id, :logdate], partition_definitions: [{ name: "p0", values: {:modulus=>3, :remainder=>0} }]
      ERB
    end
    ...
    let(:expected_dsl) do
      erbh(<<-ERB)
        ...
        add_partition "hash_partitions", :hash, [:id, :logdate], partition_definitions: [{ name: "p0", values: {:modulus=>3, :remainder=>0} }, {name: "p1", values: {:modulus=>3, :remainder=>1} }]
      ERB
    end
